...
class NBT {
    ...
    /**
     * Get the file handle resource for the provided $filename.
     *
     * @param string|resource $filename A filename string to the desired file.
     * @param string $wrapper The stream wrapper to open the file handle. Must be null if the provided $filename is a resource.
     * @param string $mode The mode used to open the file handle. {@see http://php.net/manual/en/function.fopen.php}
     */
    ...
    public function readType($fp, $tagType) {
        switch ($tagType) {
            ...
            case self::TAG_LIST: // List
                ...
                for ($i = 0; $i < $listLength && feof($fp) === false; $i++) {
    ...
    private function writeLongValue($fp, $value) {
        ...
        if (((int) 4294967296) != 0) {
            ...
            throw new Exception('Can\' write a log value.'); // There is not another way to write a long value yet
        }
    ...
    public function writeType($fp, $tagType, $value) {
        switch ($tagType) {
            ...
            case self::TAG_LIST: // List
                ...
                if ($this->writeType($fp, self::TAG_BYTE, $value['type']) === false && $this->writeType($fp, self::TAG_INT, count($value['value'])) === false) {
                    return false;
                }
